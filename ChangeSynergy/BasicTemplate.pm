 #!/usr/bin/perl -w
package ChangeSynergy::BasicTemplate;
...
=head1 NAME
  ...
  ChangeSynergy::BasicTemplate -- Simple lexical text/html/etc template parser

=cut

=head1 SYNOPSIS

 use ChangeSynergy::BasicTemplate;
 $bt = ChangeSynergy::BasicTemplate->new;
 my %dict = (
   name => 'John',
 ...
 $tmpl = "Hello, BasicTemplate.pm; your mail is in %$MAIL%. Welcome to %location%!";
 print $bt->parse(\$tmpl,\%dict);

 $tmpl = "%if new%First time, BasicTemplate.pm?%fi%".
...
B<ChangeSynergy::BasicTemplate> is a relatively straightforward template
...
ChangeSynergy::BasicTemplate attempts to be as fast and as secure as
...
If you have previously used ChangeSynergy::BasicTemplate v0.x, it is
...
picking up the apropriate syntax.  ChangeSynergy::BasicTemplate does
...
Anything to which ChangeSynergy::BasicTemplate should pay attention in
...
Example:
 ...
 $bt = ChangeSynergy::BasicTemplate->new(default_list_delimiter => ' and');
...
Subroutine references are something of a special-case in ChangeSynergy::BasicTemplate.
...
but correspond to programs internal to ChangeSynergy::BasicTemplate rather
...
a reference to the ChangeSynergy::BasicTemplate object, a hashref to the active
...
ChangeSynergy::BasicTemplate 2.0 is a major rewrite from v0.9.8 and previous
...
only variable substitution (e.g. "Hello BasicTemplate.pm") will not need
compatibility mode.
...
provided with ChangeSynergy::BasicTemplate releases.
...
Make a ChangeSynergy::BasicTemplate object.  Syntax is as follows:
...
for ChangeSynergy::BasicTemplate v0.x.  See B<COMPATIBILITY> section.
...
If true, lines staring with // will
...
sub new {
  ...
  for ('strip_html_comments','strip_c_comments','strip_cpp_comments',
       'strip_perl_comments','strip_email_comments','condense_whitespace','simple_ssi') {
      $self->{$_} ||= 0;
  }
  $self;
}
...
referenced buffer changes during the lifespan of the ChangeSynergy::BasicTemplate
...
sub parse {
	...
	if (@dict) {
	  ...
	  print STDERR "ChangeSynergy::BasicTemplate::parse($isrc): Stack underflow while flattening dictionary; odd number of elements, last was '$d'";
	}
      }
    }
    ...
    return '[ChangeSynergy::BasicTemplate::parse() recursion limit exceeded]'
...
#    print STDERR "ovr = {".join(',',map { "$_=$ovr->{$_}" } keys %{$ovr})."}";
	...
	if ($self->{use_scalarref_lexicon_cache} and
		...
		warn "ChangeSynergy::BasicTemplate::parse($isrc): File not available";
		return undef;
	    }
    ...
    $$ss =~ s/^\/\/.*?\n//mg if $self->{strip_email_comments}; ## Reed Owens change to use special // comments for email templates
    $$ss =~ s/\#.*?\n//mg if $self->{strip_perl_comments};  ## Ed Taekema change .. removed the /n blank line bug fix
...
sub load_from_file {
	...
	$self->is_tainted($fn)) {
	print STDERR "ChangeSynergy::BasicTemplate: load_from_file: '$fn' is tainted, can't open safely\n";
	return undef;
    }
	...
	$debug and print STDERR "ChangeSynergy::BasicTemplate::load_from_file($fn): $!";
	return undef;
    };
...
ChangeSynergy::BasicTemplate is by Devin Carraway <tbtpod@devin.com>, originally
