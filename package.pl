	...
	foreach $prj(@projectlist)
	{
		...
		foreach $key(keys %deliveryhash)
  		...
  		if($prj =~ /linAS5/)
  		{
  			$hostos="rhel5";
  			$hostplatform="linAS5";
  			`chmod -R 0755 *; find ./ -type f -name "*README.txt" | xargs tar cvf tertio-$mrnumber-$hostos-build$build_number\.tar; find ./ -type f  \\( ! -iname "*README.txt" \\) | xargs tar uvf tertio-$mrnumber-$hostos-build$build_number\.tar;gzip tertio-$mrnumber-$hostos-build$build_number\.tar;`;
  		...
  		{
  			$hostplatform="hpiav3";
  			`chmod -R 0755 *;  find ./ -type f -name "*README.txt" | xargs tar cvf tertio-$mrnumber-$hostplatform-build$build_number\.tar; find ./ -type f  \\( ! -iname "*README.txt" \\) | xargs tar uvf tertio-$mrnumber-$hostplatform-build$build_number\.tar; gzip tertio-$mrnumber-$hostplatform-build$build_number\.tar;`;
