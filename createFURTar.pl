...
sub createTar()
{
  ...
  if($coreproject =~ /linAS5/)
  {
  	...
  	chdir($destdir);
  	copy("$Bin/FUR_4.0_README.txt",$destdir);
  	$hostos="rhel5";
  	$hostplatform="linAS5";
	...
	elsif($coreproject =~ /sol10/)
	{
  	...
  	copy("$Bin/FUR_4.0_README.txt",$destdir);
  	$hostplatform="sol10";
  	`find * -type f -name "*README.txt" | xargs tar cvf fur-$mrnumber-$hostplatform-build$build_number\.tar; find * -type f  \\( ! -name "*README.txt" ! -name "*.tar" \\) | xargs tar uvf fur-$mrnumber-$hostplatform-build$build_number\.tar;gzip fur-$mrnumber-$hostplatform-build$build_number\.tar;`;
  	print "fur-$mrnumber-$hostplatform-build$build_number\.tar\.gz => $destination/$hostplatform/NotTested/fur-$mrnumber-$hostplatform-build$build_number\_$dtformat\.tar\.gz";
  	copy("fur-$mrnumber-$hostplatform-build$build_number\.tar\.gz","$destination/$hostplatform/NotTested/fur-$mrnumber-$hostplatform-build$build_number\_$dtformat\.tar\.gz") or die("Couldn't copy to destination $!");
  	push(@location,"$destination/$hostplatform/NotTested/fur-$mrnumber-$hostplatform-build$build_number\_$dtformat\.tar\.gz");
  	print LOCATION "$destination/$hostplatform/NotTested/fur-$mrnumber-$hostplatform-build$build_number\_$dtformat\.tar\.gz \n";
	}
