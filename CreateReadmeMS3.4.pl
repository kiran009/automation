...
sub listfolderTasks()
{
	@tasks_341=`$CCM folder -show tasks '$f_341' -u -f "%task_number"`;
	#@tasks_332=`$CCM folder -show tasks '$f_332' -u -f "%task_number"`;

	print "Tasks in 3.4.1 are => @tasks_341 \n\n";
	#print "Tasks in 3.3.2 are => @tasks_332 \n\n";
	foreach $task(@tasks_341)
	{
		$task=~ s/^\s+|\s+$//g;
	...
	@uniq341 = do { my %seen; grep { !$seen{$_}++ } @crs_341};
	...
	close BN;
	open  FILE, "+> $Bin/MS_3.4_README.txt";
	print FILE "Maintenance Release : MS 3.4.1 build $build_number\n\n";
	print FILE "Created: $dt\n\n";
	print FILE "PRE-REQUISITE : 3.4.0\nSUPERSEDED : None \n";
	undef @tasks;
	open MR,"+> $Bin/mrnumber.txt";
	print MR "3.4.1";
	close MR;
	open SYNOP,"+>$Bin/3.4.1_synopsis.txt";
	open SUMM,"+>$Bin/3.4.1_summary_readme.txt";
	open CRRESOLV, "+>$Bin/3.4.1_crresolv.txt";
	open TASKINF,"+>$Bin/3.4.1_taskinfo.txt";
	open PATCHBIN, "+>$Bin/3.4.1_patchbinarylist.txt";
	open FORMATTASKS,"+>$Bin/3.4.1_formattsks.txt";
	open FOLDER,"+>$Bin/3.4.1_folder.txt";
	print FOLDER $f_341;
	close FOLDER;
	getTasksnReadme(@uniq341);
	close SUMM;
	close SYNOP;
	close CRRESOLV;
	close TASKINF;
